        ...
        private static async Task<IResult> ChangeVideoSpeed(HttpContext context, [FromForm] ChangeSpeedDto dto)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (!result.IsSuccess)
                    {
                        logger.LogError("Speed change failed: {Error}", result.ErrorMessage);
                        return Results.Problem("Speed change failed: " + result.ErrorMessage);
                    }

                    byte[] output = await fileService.GetOutputFileAsync(outputFileName);
                    _ = fileService.CleanupTempFilesAsync(filesToCleanup);
                    return Results.File(output, "video/mp4", outputFileName);
                }
                catch (Exception ex)
                {
        ...
        private static async Task<IResult> ApplyColorFilter(HttpContext context, [FromForm] ColorFilterDto dto)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (!result.IsSuccess)
                    {
                        logger.LogError("FFmpeg color filter command failed: {ErrorMessage}, Command: {Command}", result.ErrorMessage, result.CommandExecuted);
